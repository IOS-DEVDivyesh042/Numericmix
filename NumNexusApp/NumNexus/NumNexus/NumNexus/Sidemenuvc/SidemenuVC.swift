...
class SidemenuVC: UIViewController {
    ...
    var arrTblList = ["FEEDBACK" , "SHARE" , "PRIVACY", "TERMS ","HELPSUPPORT","RATE","ABOUTUS","HOW TO USE","ArmStrong","Fibonacci","Factortial","Palindrome"]
         //var arrImg = ["1" , "2" , "3", "5","4","7","8","6"]
...
extension SidemenuVC : UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        UIApplication.shared.openURL(NSURL(string: sUrl)! as URL)
            ...
            let vc = self.storyboard?.instantiateViewController(withIdentifier: "Armdesc") as! Armdesc
            self.navigationController?.pushViewController(vc, animated: true)
            
        }else if indexPath.row == 9{
            let vc = self.storyboard?.instantiateViewController(withIdentifier: "Fibodesc") as! Fibodesc
            self.navigationController?.pushViewController(vc, animated: true)
            
        }
        else if indexPath.row == 10{
            let vc = self.storyboard?.instantiateViewController(withIdentifier: "Factodesc") as! Factodesc
            self.navigationController?.pushViewController(vc, animated: true)
            
        }
        else if indexPath.row == 11{
            let vc = self.storyboard?.instantiateViewController(withIdentifier: "Palidesc") as! Palidesc
            self.navigationController?.pushViewController(vc, animated: true)
